...
import { Lock, Gamepad2, BookOpen, Microscope, Save, Power, LayoutDashboard, Users } from 'lucide-react';
...
export default function AdminDashboard() {
  ...
  const [visitorCountToday, setVisitorCountToday] = useState<number>(0);
  ...
  useEffect(() => {
    ...
    const fetchVisitors = async () => {
      try {
        ...
        const data = await res.json();
        if (data.summary) {
          const total = data.summary.reduce((acc: number, curr: any) => acc + curr.count, 0);
          setVisitorCount(total);
          setVisitorCountToday(data.todayCount || 0);
        }
      } catch (e) {
        ...
        <div className="flex items-center gap-6">
          ...
          <div className="hidden md:flex items-center gap-3 bg-slate-50 px-4 py-2 rounded-xl border border-slate-100">
            ...
            <div className="flex flex-col">
              <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest leading-none mb-0.5">Total Visitors</span>
              <span className="text-sm font-black text-slate-800 leading-none">{visitorCount.toLocaleString()}</span>
            </div>
          </div>
          ...
          <div className="hidden md:flex items-center gap-3 bg-orange-50 px-4 py-2 rounded-xl border border-orange-100">
            ...
            <div className="flex flex-col">
              <span className="text-[10px] font-black text-orange-400 uppercase tracking-widest leading-none mb-0.5">Today</span>
              <span className="text-sm font-black text-orange-600 leading-none">{visitorCountToday.toLocaleString()}</span>
            </div>
          </div>
        </div>
        <button 
